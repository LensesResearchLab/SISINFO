...
@Injectable()
export class AlertsProjectService {
  constructor(
    @InjectRepository(AlertProject)
    private alertProjectRepository: Repository<AlertProject>,
  ) {}

  async create(createAlertProjectDto: CreateAlertProjectDto) {
    const alertProject = this.alertProjectRepository.create(
      createAlertProjectDto,
    );
