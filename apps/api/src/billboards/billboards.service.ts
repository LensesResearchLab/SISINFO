...
export class BillboardsService {
  ...
  async create(createBillboardDto: CreateBillboardDto[]) {
    ...
    for (const info of createBillboardDto) {
      ...
      let periodFound = await this.periodsService.findOneByPeriodAndYear(periodStr, year);
      if (!periodFound) {
      ...
      let existingCourse = await this.courseService.findByCodeAndPeriod(info.code, periodFound) as Course | null;
      let existingSection = await this.sectionService.findByNRCAndPeriod(info.NRC, periodFound);
  
      if (info.professors != null && info.professors !== "") {
        const professorsArr = info.professors.split("|");
        let supportProfessors: Professor[] = [];
        let findedProfessors: Professor[] = [];
        ...
        await Promise.all(
          professorsArr.map(async (prof) => {
            ...
            if (searchProfessor) {
              if (/\(01\)/.test(prof)) {
                if (!findedProfessors.find(p => p.document === searchProfessor.document)) {
                  findedProfessors.push(searchProfessor);
                }
              } else if (/\(02\)/.test(prof)) {
                if (!supportProfessors.find(p => p.document === searchProfessor.document)) {
        ...
        if (existingSection != null) {
          ...
          sectionToUse = await this.sectionService.create(newSectionDto, supportProfessors, findedProfessors, periodFound) as Section;
          billboardSectionsMap.set(sectionToUse.NRC, sectionToUse);
        }
  
        if (existingCourse) { 
          const sectionAlreadyIncluded = existingCourse.sections?.some(sec => sec.NRC === sectionToUse.NRC);
          if (!sectionAlreadyIncluded) {
            existingCourse = await this.courseService.updateSections(sectionToUse, existingCourse);
          }
          billboardCoursesMap.set(existingCourse.code, existingCourse);
        } else {
          let courseDto = new CreateCourseDto();
    ...
    let periodForBillboard = await this.periodsService.findOneByPeriodAndYear(firstPeriodStr, billboardYear);
    ...
    let billboardExisting = await this.findOne(billboard.period.year + billboard.period.period);
  ...
  findAll() {
    return this.billboardRepository.find({ relations: ["courses", "courses.sections", "courses.sections.professors", "courses.sections.period"] });
  }

  async findOne(period: string) {
    const year = Number(period.slice(0, 4));
    const periodDigit = period.slice(4, 6);
    const periodBillboard = await this.periodsService.findOneByPeriodAndYear(periodDigit, year);
    if (!periodBillboard) {
      throw new Error("Period not found");
    }
    const billboard = await this.billboardRepository.findOne({
      where: { period: periodBillboard },
      relations: ["courses", "courses.sections", "courses.sections.professors", "courses.sections.period"],
    });
    return billboard;
  }
