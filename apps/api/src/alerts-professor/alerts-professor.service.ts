...
@Injectable()
export class AlertsProfessorService {
  constructor(
    @InjectRepository(AlertProfessor)
    private alertProfessorRepository: Repository<AlertProfessor>,
  ) {}
  async create(createAlertProfessorDto: CreateAlertProfessorDto) {
    const alertProfessor = this.alertProfessorRepository.create(
      createAlertProfessorDto,
    );
