import { Assistance, StatusInformation } from "@/app/home/graduated-assistance/types/assistance.type";
...
export async function getAssistanceStatusById(id: string) {
  await new Promise((resolve) => setTimeout(resolve, 1500));
      const application = status.find(application => application.id === Number(id));
      if (application) {
          return application;
      }
